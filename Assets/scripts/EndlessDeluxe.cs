...
[System.Serializable]
public class EDlevelsegment
{
    public int levelseg;
    public int objseg;
}
public class EndlessDeluxe : MonoBehaviour
{
    ...
    public EDlevelsegment[] levelorder;  //class at bottom holding int int level id and object id
    ...
    private void Awake()
    {
      //init
        ...
        foreach (var g in LevelSegments)
        {
            ...
            roadtile_ED rt = g.AddComponent<roadtile_ED>();         
    ...
    //this is the reusable function to make a new level segment at x dis
     GameObject spawnnewsegement(EDlevelsegment ls, float dis)
    {
        //spawn the level
        type = 0;
        ...
        if (ls.objseg > -1)
        {
            ...
            lvlseg.transform.GetComponent<roadtile_ED>().setspeed( ls.levelseg, ls.objseg, obj);
        }
        else
        {
            //hide if exists
            lvlseg.transform.GetComponent<roadtile_ED>().setspeed(ls.levelseg,ls.objseg,null);
        }
...
class roadtile_ED : MonoBehaviour
{

    public EndlessDeluxe master;
